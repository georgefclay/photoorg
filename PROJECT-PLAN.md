...
- **Upload page**, mobile-first: camera / gallery picker on phones; drag-drop and folder picker (`webkitdirectory`) on desktop; sequential uploads with per-file progress, retry, and resume; batch note. **Admin approval page**: thumbnails, uploader, dup warnings, approve/reject per file or batch.
